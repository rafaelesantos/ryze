//
//  RefdsNetworkSocketAdapter.swift
//  Refds
//
...
public actor RefdsNetworkSocketAdapter: RefdsNetworkSocketClient {
    ...
    public func connect<Request: RefdsNetworkSocketRequest>(with request: Request) async throws -> AsyncThrowingStream<String, Error> {
        guard let endpoint = await request.endpoint else {
            logger.error("❌ Invalid URL for request: \(String(describing: request))")
            throw RefdsNetworkError.invalidURL
        }
    ...
    private func connect() async throws -> AsyncStream<RefdsNetworkSocketStatus> {
    ...
    public func send(message: String) async throws {
        guard let content = message.breakLine.data(using: .utf8) else {
            ...
            throw RefdsNetworkError.badRequest
        }
